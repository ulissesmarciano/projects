...
const HomeScreen = () => {
  ...
  const username: any = getUsername.user

  const getUser = async (username: IHomeScreen) => {
    const details = await getUserData(username)
    setUser(details.data)
    setLoading(false)
  }

  const getRepos = async (username: IHomeScreen) => {
    const details = await getReposData(username)
    setRepos(details.data)
  }

  const getStarred = async (username: IHomeScreen) => {
    const details = await getStarredData(username)
    setStarred(details.data)
  }

  const getFollowers = async (username: IHomeScreen) => {
    const details = await getFollowersData(username)
    setFollowers(details.data)
  }

  const getFollowing = async (username: IHomeScreen) => {
  ...
  const getUserData = async (username: IHomeScreen) => {
    const res = await axios.get(`https://api.github.com/users/${username}`)
    return res

  }

  const getReposData = async (username: IHomeScreen) => {
    const res = await axios.get(`https://api.github.com/users/${username}/repos`)
    return res
  }

  const getStarredData = async (username: IHomeScreen) => {
    const res = await axios.get(`https://api.github.com/users/${username}/starred`)
    return res
  }

  const getFollowersData = async (username: IHomeScreen) => {
    const res = await axios.get(`https://api.github.com/users/${username}/followers`)
    return res
  }

  const getFollowingData = async (username: IHomeScreen) => {
